// supabase/functions/_shared/email_lint.ts
//
// Email HTML linter — server-authoritative checks run before INSERT to
// email_templates / email_campaigns. Mirrored client-side (src/lib/emailLint.ts)
// for instant feedback in the admin UI, but the server is the source of
// truth (we block the save on 'error' severity).
//
// Design: pure functions, regex-based heuristics. No DOM parser so this
// file runs identically in Deno (edge functions) and a browser (via a
// thin re-export). Heuristics are calibrated to "real" campaign / template
// HTML written in the Monaco editor + block editor — false positive rate
// is more important than catching every possible issue.
//
// Severity levels:
//   - error   → blocks save. Reserved for structural problems that will
//               break rendering on real mail clients.
//   - warning → accepts save but surfaces prominently. Deliverability or
//               accessibility issues that hurt open/spam scores.
//   - info    → shown in details panel. Nice-to-have quality signals.
...
export interface LintInput {
  subject: string;
  html: string;
  /** Admin-declared variables for the template — used to detect orphans. */
  declaredVariables?: Array<{ key: string } | string>;
  /** 'transactional' | 'campaign'. Campaign requires an unsubscribe placeholder. */
  category?: string;
}
...
const LIMITS = {
  HTML_BYTES_WARN: 102_400,       // Gmail clips at ~102KB → "View full email" link
...
export function lintEmail(input: LintInput): LintResult {
  ...
  if (!subject) {
    ...
    if (subject.length > LIMITS.SUBJECT_CHARS_ERROR) {
      issues.push({
  ...
  // --- HTML size (bytes, not chars — emails ship bytes) ------------------
  ...
  // --- <img> accessibility + spam-filter signals ------------------------
  ...
  // Reserved render-time variables that the render pipeline fills in;
  // admins don't need to declare them.
  ...
  // --- Campaign-only: unsubscribe placeholder required -------------------
...
// ========================================================================
// Tag balance — heuristic HTML validator
// ========================================================================
